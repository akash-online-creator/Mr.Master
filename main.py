...
def load_data():
    default_state = {
        ...
        'first_win_list': [],  # Scanner එකෙන් එන මුල් කාසි ලැයිස්තුව         
        'first_win_coins': [], # # පළමු වතාවෙම (Step 0) WIN වන කාසි ලැයිස්තුව
        'shared_loss_buffer': 0.0,       
        'shared_loss_splits': 0,         
        'total_loss_cost': 0.0, # load_data ශ්‍රිතය ඇතුළත default_state එකට මෙය එක් කරන්න
                        ...
                        if not is_ict_trading_window():
                            with state_lock:
                                ...
                                if next_step >= 4:
                                    ...
                                    state['symbol_recovery_step'][s] = 0
                                    state['symbol_accumulated_loss'][s] = 0.0
                                    execute_telegram_send(f"❌ <b>RECOVERY FAILED ({s})</b>\nකාලය අවසන් වන විට පියවර 3ම අසාර්ථක වී තිබූ නිසා මෙම කාසිය Blacklist කරන ලදී.")
                                else:
                            ...
                            execute_telegram_send(f"🛑 <b>TIME WINDOW OVER ({s})</b>\nසිග්නල් කාලය අවසන් බැවින් ට්‍රේඩ් එක නතර කළා. මීළඟ Window එකේදී Step {next_step} එකෙන් නැවත ආරම්භ වේ.")
                            sync_save()
                            continue

                        # --- සාමාන්‍ය වෙලාව ඇතුළත SL වැදුණු විට රිකවරි ට්‍රේඩ් එකක් දැමීම ---
                        with state_lock:
                            state['stats']['loss'] += 1
                            state['daily_stats']['loss'] += 1
                            state['daily_stats']['loss_amount'] += trade_loss
                            next_step = pos['step'] + 1
                            current_total_loss = state['symbol_accumulated_loss'].get(s, 0.0) + trade_loss
                            ...
                            if next_step >= 4:
                                ...
                                if s not in state['daily_stats']['blacklist_coins']: state['daily_stats']['blacklist_coins'].append(s)
                                
                                trade_fee = pos['margin'] * 0.0004
                                total_failure_cost = current_total_loss + trade_fee
                                state['total_loss_cost'] += total_failure_cost
                                state['symbol_recovery_step'][s] = 0
                                state['symbol_accumulated_loss'][s] = 0.0
                                execute_telegram_send(f"❌ <b>RECOVERY FAILED ({s})</b>\nපියවර 3ම අසාර්ථක විය! මෙම කාසිය Blacklist කරන ලදී.")
                            else:
                                state['symbol_recovery_step'][s] = next_step
                                state['symbol_accumulated_loss'][s] = current_total_loss
                                threading.Thread(target=execute_new_trade, args=(s, reverse_side, current_p)).start()
                        sync_save()
                except: pass
                time.sleep(0.2)
            time.sleep(1)
        ...
        if str(chat_id).strip() == str(TELEGRAM_CHAT_ID).strip() and raw_text:
            ...
            if cmd == "bot_on":
                ...
                menu_msg = ("📜 <b>RED BULL MASTER COMMANDS MENU</b>\n━━━━━━━━━━━━━━━━━\n"
                            ...
                            "• /reset_tradesss : පවතින Active Trades දත්ත පද්ධතියෙන් මකා දමයි.අගට ඇති ss අකුරු දෙක ඉවත්කර ලබාදෙන්න.")
